...
const Resume = (props) => {
    ...
    const { personalData, education, experience, project, skills } = props;
    console.log({ personalData, education, experience, project, skills });
    return (
        <div>
            ...
            <div className="resume-footer">
                {false && (
                    <button className="btn warn resume-btn" onClick={props.download}>
                        Download PDF [NOT WORKING]
                    </button>
                )}
                <button className="btn del resume-btn" onClick={reset}>
                    Reset
                </button>
            </div>
        </div>
